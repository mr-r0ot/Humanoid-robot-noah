...
def Cnet():
    ...
    if NetWork.Check_Net()==False:
        connect_wifi()  
        time.sleep(5)  
        if check_wifi_connection():
            print("Connected to wifi")
            Talk('Oh well, I am connected to the network!')
        else:
            print("test defult Password")
            TalkOffline('Wifi have a password i try it.')
            password_list = [
                "12345678",
                "nova1234",
            ]
            for pwd in password_list:
                TalkOffline(f"Test: {pwd}")
                connect_wifi(pwd)
                time.sleep(5)
                if check_wifi_connection():
                    Talk('Oh well, I am connected to the network!')
                    ...
                    TalkOffline(f"{pwd} is incorrect")
            else:
                TalkOffline("None of the passwords were correct.")
                TalkOffline("Please create a wifi named nova and leave it blank or use the password 12345678 or nova1234")
                TalkOffline("And turn me off and then on.")
                sys.exit()
    else:
        Talk('Oh well, I am connected to the network!')
Cnet()
