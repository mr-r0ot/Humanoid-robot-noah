    ...
    with microphone as source:
        ...
        while True:
            try:
                ...
                if transcript and (time.time() - last_update_time) > 3:
                    print('Listen Finish')
                    return transcript
            except sr.UnknownValueError:
                print("متن قابل فهمی تشخیص داده نشد.")
            except sr.RequestError as e:
                print("خطا در درخواست سرویس؛", e)
...
"""
    ...
    try:
        apis=['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','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']
        for api in apis:
            client = OpenAI(
        ...
        try:
            response = requests.post("https://persianaichat.ir/private_chatgptAPI_mvfhvehbvebvbetkbrkvb643vg.php", json=payload)

            if response.status_code == 200:
                result = response.json()
                if "choices" in result and len(result["choices"]) > 0:
                    return json.loads((result["choices"][0].get("message", {}).get("content", "")).replace('```json','').replace('```',''))
                else:
                    return False
            else:
                return False
